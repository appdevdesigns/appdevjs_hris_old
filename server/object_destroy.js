...
/**
 * @class hris.server.Object.destroy
 * @parent hris.server.Object
 *
 * Performs the destroy action for a defined object in the system.
 *
...
//    var tInfo = req.aRAD.tableInfo;
    ...
    var permission = 'hris.'+objectKey+'.destroy';
...
var postNotifications = function (req, res, next) {
    ...
    var notification = 'hris.object.'+objectKey+'.destroyed';
...
var serviceURL = publicLinks.destroy.uri.replace('[id]',':id').replace('[object_key]', ':hrisObjKey');
...
hrisObjectCreate.setup = function( app ) {
    ...
    var objectStack = [
           ...
           HRiS.Express.destroyObject, // perform the actual db destroy operation
...
//           changedValues,             // find any changed values that need to be reported back
	...
	////---------------------------------------------------------------------
	app.delete(serviceURL, objectStack, function(req, res, next) {
//app.get('/hris/destroy/:hrisObjKey/:id', objectStack, function(req, res, next) {
	    ...
	    logDump(req, 'finished '+serviceURL.replace(':hrisObjKey', objectKey)+' (destroy) ');
